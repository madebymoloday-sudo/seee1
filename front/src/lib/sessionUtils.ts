...
export function parseImportantOptions(text: string): string[] {
  ...
  // 1) Базовое разбиение по структурным разделителям (включая запятую — частый разделитель идей)
  const coarse = normalized
    .split(
      /\r?\n|,\s*|;\s*|•\s*|\u2022\s*|\s+—\s+|\s+-\s+(?=\S)|\*\s*|\d+[\)\.]\s+|(?:Второе|В третьих|Во-первых|Первое|Третье|Во-вторых)\s*[—\-]\s*/gi
    )
