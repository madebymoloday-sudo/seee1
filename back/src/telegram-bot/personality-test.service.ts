...
Важно: в начале сообщения не пиши заголовок «Часть 1» — его добавим отдельно. Каждый пункт начинай с жирной буквы — именно в формате **А)**, **Б)**, **В)**, **Г)**, **Д)**, **Е)**, **Ж)**, **З)** (буква и скобка жирным через **). После скобки — название блока, его тоже выдели жирным. Между блоками оставляй пустую строку для наглядного разделения.
...
Тон: тёплый, поддерживающий, без осуждения. Опирайся только на приведённые ответы. Пиши развёрнуто, но структурированно. Все заголовки пунктов (А)–З)) обязательно в формате **А)**, **Б)** и т.д.`;
  ...
  async generate4Points(answers: Record<string, string | number>): Promise<string> {
    ...
    const systemPrompt = `Ты — психолог в стиле Seee. На основе ответов пользователя сформируй 4 блока — по одному на сферу. Заголовок «Часть 2» не пиши — его добавят отдельно.

Для каждой сферы выведи ровно 4 подпункта, каждый с жирным подзаголовком (обязательно **жирным** через **):
**N. Название сферы** (например **1. Секс**)
...
**Что рекомендую разобрать в Seee:** внутри этого пункта тоже выделяй жирным подпункты: **а)** Зачастую проблемы в этой сфере связаны не с внешними условиями, а с ограничивающими убеждениями... **б)** Чаще всего встречаются такие убеждения, которые формируют чувство [типичные для сферы]. **в)** В твоём случае рекомендую разобрать в Seee следующие карточки: «карточка 1», «карточка 2», «карточка 3». В конце одна фраза: это поможет начать движение, потому что в голове хранятся все команды, а мозг управляет нашим поведением.

Сферы по порядку: Секс, Реализация, Здоровье, Отношения. Тон: поддерживающий, мотивирующий, бережный. Между блоками сфер — одну пустую строку. Все подзаголовки (**Что получается:**, **Что доработать:**, **Возможные последствия:**, **Что рекомендую разобрать в Seee:**, **а)**, **б)**, **в)**) обязательно выделяй жирным.`;
