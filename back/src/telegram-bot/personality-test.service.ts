...
export class PersonalityTestService {
  ...
  async generateExtraFeedback(answers: Record<string, string | number>): Promise<string> {
    ...
    const systemPrompt = `Ты — психолог в стиле Seee. На основе только этих ответов пользователя (на вопросы не про 4 сферы жизни) напиши один связный текст обратной связи. Обязательно включи все блоки по пунктам, с заголовками **...**:
...
Тон: тёплый, поддерживающий, без осуждения. Опирайся только на приведённые ответы. Пиши развёрнуто, но структурированно. Используй ** только для заголовков блоков.`;
