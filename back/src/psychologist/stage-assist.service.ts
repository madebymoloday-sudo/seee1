...
export class StageAssistService {
  ...
  private getAdvanceReaction(
    ...
    switch (previousStep) {
      case 1:
        return 'Спасибо, теперь ситуация стала понятнее. Давайте посмотрим, какую эмоцию она у вас вызывает.';
      case 2:
        if (this.isPositiveEmotion(emotion)) {
          return 'Хорошо, что вы замечаете и такие чувства тоже. Давайте теперь найдём мысль, которая запускает эту эмоцию.';
        }
        return 'Понимаю, это может быть непросто. Давайте теперь найдём мысль или идею, которая запускает эту эмоцию.';
      case 3:
        return 'Хорошо, мысль уже стала заметнее. Теперь разложим, почему она кажется вам убедительной.';
      case 4:
        return 'Спасибо, причины уже проявились. Теперь посмотрим, от кого или откуда вообще могла прийти сама мысль.';
      case 5:
        return 'Это помогает увидеть контекст, в котором мысль закрепилась. Давайте теперь посмотрим, какую выгоду она могла давать источнику.';
      case 6:
        return 'Это важное наблюдение. Теперь посмотрим, что эта мысль делает с вами эмоционально.';
      case 7:
        return 'Спасибо, внутренний эмоциональный след уже хорошо виден. Давайте посмотрим, как мысль влияет на вашу жизнь на практике.';
      case 8:
        return 'Теперь уже видно, как эта мысль отражается и на чувствах, и на реальных действиях. Осталось понять, нужна она вам или нет.';
      case 9:
